...
| 2026-04-27 night | **Venue LOCKED: IEEE TMC primary / OJ-COMS fallback** after 3-round deep lit review. **New paper narrative**: "Test the separability assumption — prior work treats arch/algo/heterogeneity/hardware as separable axes; we expose joint interactions on Colosseum/ColO-RAN with first 6D integration benchmark." **Phase 1.5g infrastructure complete** (5 commits `87cf12c`/`608f347`/`2dbd1eb`/`54863a8`/`0131d78`, 35 new tests, 404 total). **Phase 2 minimum done** (36/36 cells, 18 fedavg + 18 fedprox retry, 26.6 min). **Phase 3 plan FINAL**: drop 3b (preempted by arXiv 2602.12009 Feb 2026 DP×SNN×FL); drop 3d (weak); add 3e (failure-envelope α∈{0.01, 0.05, 0.1, 0.5, 1, 5}); revise 3a to parametric α∈{0.05, 0.1}. **Phase 5 NEW = Stage 2 FULL 750-cell sweep** (3 archs × 5 algos × 5 alphas × 10 seeds, ~16 hr GPU). Timeline 8-12 weeks. NeurIPS ED dropped (rushed + ML reviewer pool not aligned). Croissant metadata + Zenodo DOI + 4080 reference baselines as open release. New `docs/PAPER_CONTRIBUTION_CLAIM.md` is canonical contribution claim doc. | 3-round comprehensive lit review confirmed niche still ours via 6D intersection {LSTM + Mamba + Spiking-SSM} × {5 FL algos} × {parametric Dirichlet} × {ColO-RAN testbed} × {NVML in FL setting} × {paired-bootstrap CI95}. New citations: Shen 2023, NeuroBench 2025, SpikingBrain 2025, Prosperity 2025, ML.ENERGY 2025, FedLEC 2024, Privacy-FL-SNN 2025, arXiv 2602.12009 2026, Mangi 2026, embedding-skew 2025, fev-bench 2025, 6G-Bench 2026. Polese (ColO-RAN dataset author) published in TMC + OJ-COMS → community fit signal. Honest down-shift from "novel finding" framing to "first comprehensive integration + separability assumption test" is more defensible. |
