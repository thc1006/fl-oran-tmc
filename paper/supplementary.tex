...
\subsection{Applicability boundary (extends \S\,7.1)}\label{app:applicability}

The apparent inversion is a \emph{sequence-integrity artifact} of row-level client partitioning (\S\,7.1, run-level controls), not a dataset-structural property; its scope is therefore a property of \emph{benchmark construction} rather than of RAN telemetry. It replicates whenever a federated benchmark (a)~pools a time series and synthesises client heterogeneity by row-level partitioning (Dirichlet-over-samples or uniform random assignment --- the de-facto FL non-IID default), and (b)~constructs per-client sliding windows \emph{after} partitioning, so the row split scatters each entity's consecutive timesteps and corrupts the windows. Deployments and benchmarks that partition by natural entity (each gNB / run kept intact) never incur it and behave classically --- the natural partition recovers full AUC because it preserves sequences, not because RAN telemetry has special cell-conditional structure. We conjecture other federated time-series benchmarks that partition by row exhibit the same spurious inversion; auditing them with the run-level controls and the window-contiguity check of \S\,7.1 is open future work.

\subsection{Hardware drift caveat for the \S\,7.1 \lstinline|random_split| ablation (extends \S\,7.1)}\label{app:hardware-drift}
...
We compute the BCE positive-class weight from the train partition's positive rate (per ADR-001 D-12), in line with standard practice that prevents test-set positive-rate from leaking into the loss. Two alternative computations (val-derived \lstinline|pos_weight|; held-out-fold \lstinline|pos_weight|) would also be valid and might shift absolute AUC numbers slightly, but the BCE loss reweighting is identical (globally pooled) across all clients regardless of which split the rate is computed from, so the \emph{direction} of all our findings is invariant to this choice. (The inverted-$\alphadir$ itself is a sequence-integrity artifact, \S\,7.1; it does not depend on the \lstinline|pos_weight| split either.)
...
The $10\%$ BLER threshold is the canonical ColO-RAN SLA gate~\cite{Polese2022}; the empirical positive rate at this threshold on the train partition is $30.9\%$ (\S\,7.1 setup-level facts). Since the inverted-$\alphadir$ is a sequence-integrity artifact (\S\,7.1) --- per-client window corruption, independent of the label threshold --- we expect the corrected (run-level) result to be \emph{threshold-invariant}: run-level partitions should match natural-by-BS at every threshold, while the row-level deficit should persist (it is windowing, not labels). A $5\%$ / $15\%$ / $20\%$ sweep would confirm this threshold-invariance of the corrected finding; we did not run it. (The earlier prediction that ``inversion strengthens as the threshold tightens'' is withdrawn --- it presumed a structural mechanism that the run-level controls refute.)
...
Several open questions remain. \textbf{First}, the run-level controls (\S\,7.1) resolve the inverted-$\alphadir$ as a sequence-integrity artifact, but a small (${\sim}0.002$ AUC) residual FL-heterogeneity cost remains for run-level Dirichlet at the most-skewed $\alphadir$; cleanly isolating it from per-round-participation confounds (the \lstinline|sub_per_bs=2| / $5/14$-sampling-fraction design of the per-BS ablation) --- e.g.\ an isolation re-run bumping \lstinline|clients-per-round| to match Phase~5's $5/7$ fraction --- is minor open work, following the non-IID-impact methodology of Jimenez-Gutierrez et al.~\cite{JimenezGutierrez2025_NonIID} and ProFed~\cite{ProFed2025}. \textbf{Second}, our $10\%$ BLER threshold should be perturbed to $\{5, 15, 20\}\%$ to confirm operational robustness, in line with recent rare-event metric-stability work~\cite{arXiv2504_16185}. \textbf{Third}, scaling beyond $N = 7$ base stations to the larger-$N$ 6G slicing and edge-FL regimes studied by Chiarani et al.~\cite{Chiarani2025} and Sezgin et al.~\cite{Sezgin2025} would test whether the corrected finding --- that FL is robust to client heterogeneity once sequences are kept intact --- continues to hold at large $N$. \textbf{Fourth}, the FedAdam headroom-saturation finding may be soft: variance-aware sharpness methods such as FedSCAM~\cite{FedSCAM2026} and synthetic-data-guided FedSynSAM~\cite{FedSynSAM2026} introduce per-client perturbation modulation that could extract additional accuracy. \textbf{Fifth}, integrating differential privacy via over-the-air channel noise~\cite{arXiv2510_23463} and exploring neuromorphic edge deployment --- both sparse-RNN acceleration on Loihi-class accelerators~\cite{arXiv2502_01330} and spiking-NN low-power edge sensing~\cite{arXiv2604_27004} --- would close the loop toward production-deployable privacy-preserving energy-efficient O-RAN slice prediction.
